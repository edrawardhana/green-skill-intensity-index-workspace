...
import { Cpu, Sparkles, Plus, AlertCircle, CheckCircle2, Award, Zap, ArrowRight, HelpCircle } from 'lucide-react';
...
export default function NlpPlayground({ cleanedJobs, onProcessAllJobs, processedJobs }: NlpPlaygroundProps) {
  ...
  const [activeNlpMode, setActiveNlpMode] = useState<'preset' | 'custom'>('preset');
  ...
  const runNlpExtraction = async (title: string, description: string): Promise<{ skills: Skill[]; provider: string }> => {
    try {
  ...
  const handleProcessSingle = async () => {
    const title = activeNlpMode === 'preset' ? (cleanedJobs.find(j => j.job_id === selectedJobId)?.job_title || '') : customTitle;
    const desc = activeNlpMode === 'preset' ? (cleanedJobs.find(j => j.job_id === selectedJobId)?.job_description || '') : customDesc;
    ...
    setSingleResult({
      title,
      company: activeNlpMode === 'preset' ? (cleanedJobs.find(j => j.job_id === selectedJobId)?.company || 'Sektor Kustom') : 'PT Industri Kustom',
    ...
    <div className="space-y-6" id="nlp-playground-root">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
        ...
        <p className="text-slate-600 text-sm leading-relaxed">
          Kekuatan utama riset ini terletak pada ekstraksi skill berbasis teks (<strong className="text-emerald-700">NLP Skill Extraction</strong>) 
          menggunakan model kecerdasan buatan. Kami memetakan kompetensi yang terdeteksi ke Green Ontology dan 
          menghitung <strong className="text-emerald-700">Green Skill Intensity Index</strong> untuk setiap lowongan.
        </p>
      </div>
      ...
      {cleanedJobs.length === 0 ? (
        <div className="p-8 text-center bg-slate-50 rounded-2xl border border-dashed border-slate-200">
          ...
          <p className="text-slate-600 font-semibold text-sm">Tidak Ada Dataset Bersih untuk Diproses</p>
          <p className="text-slate-400 text-xs mt-1">Harap jalankan Fase 2: Pembersihan Data terlebih dahulu agar teks terstandarisasi.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-5 bg-white rounded-2xl border border-slate-100 p-5 shadow-sm space-y-4">
            <h3 className="font-bold text-slate-800 text-sm border-b border-slate-100 pb-2">Single Job NLP Sandbox</h3>
            
            <div className="flex gap-2 bg-slate-100 p-1 rounded-xl">
              <button
                onClick={() => setActiveNlpMode('preset')}
                className={`flex-1 py-1.5 text-xs font-semibold rounded-lg transition-all ${
                  activeNlpMode === 'preset' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                }`}
              >
              ...
              <button
                ...
                className={`flex-1 py-1.5 text-xs font-semibold rounded-lg transition-all ${
                  activeNlpMode === 'custom' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                }`}
              >
            ...
            {activeNlpMode === 'preset' ? (
              <div className="space-y-1">
                <label className="text-xs font-medium text-slate-500 block">Pilih Lowongan dari Dataset Bersih</label>
                <select
                  value={selectedJobId}
              ...
              <div className="space-y-3">
                <div className="space-y-1">
                  <label className="text-xs font-medium text-slate-500 block">Judul Pekerjaan</label>
                  <input
                    type="text"
                ...
                <div className="space-y-1">
                  <label className="text-xs font-medium text-slate-500 block">Deskripsi Pekerjaan / Kualifikasi</label>
                  <textarea
                    rows={5}
            ...
            <button
              ...
              disabled={isProcessingSingle || (activeNlpMode === 'preset' && !selectedJobId) || (activeNlpMode === 'custom' && !customTitle.trim())}
            ...
            <div className="border-t border-slate-100 pt-4 space-y-3.5">
              <h4 className="font-bold text-slate-800 text-xs uppercase tracking-wider text-slate-400">Pemrosesan Massal (Bulk Dataset Engine)</h4>
              <p className="text-slate-500 text-[11px] leading-relaxed">
                Menghitung skor indeks intensitas kompetensi hijau secara massal untuk seluruh baris dataset yang telah dibersihkan. 
                Hasil ini akan digunakan untuk visualisasi dashboard dan analisis trend di Fase 4.
              </p>
              ...
              {processedJobs.length > 0 && (
                <div className="p-3 bg-emerald-50 border border-emerald-100 rounded-xl flex gap-2 items-center">
                  ...
                  <p className="text-[11px] text-emerald-800 font-medium">
                    Sukses memproses {processedJobs.length} lowongan massal! Buka Fase 4: Analisis & Dashboard untuk grafik lengkap.
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="lg:col-span-7 space-y-4">
            {singleResult ? (
              <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm space-y-4">
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2 border-b border-slate-100 pb-3">
                  <div>
                    <span className="text-[10px] font-semibold text-emerald-600 uppercase tracking-wider bg-emerald-50 border border-emerald-100 px-2 py-0.5 rounded">
                      Model Hasil: {singleResult.provider === 'gemini-3.5-flash' ? 'Gemini AI NLP' : 'Local RegEx Fallback'}
                    </span>
                    <h3 className="font-bold text-slate-800 text-lg mt-1.5 leading-tight">{singleResult.title}</h3>
                    <p className="text-xs text-slate-500">{singleResult.company}</p>
                  </div>

                  <div className="text-left sm:text-right">
                    <span className="text-[10px] font-bold text-slate-400 block uppercase">Green Intensity Index</span>
                ...
                <div className="space-y-1.5 p-3 bg-slate-50 rounded-xl border border-slate-200/60">
                  <div className="flex justify-between items-center text-xs">
                    <span className="font-medium text-slate-600">Klasifikasi Klasis</span>
                    <span className="font-bold text-slate-800">{getIntensityLevel(singleResult.intensityScore)}</span>
                  </div>
                  <div className="w-full bg-slate-200 h-2 rounded-full overflow-hidden">
                    <div 
                      className={`h-full rounded-full transition-all duration-500 ${
                        singleResult.intensityScore >= 75 ? 'bg-emerald-500' :
                        singleResult.intensityScore >= 45 ? 'bg-blue-500' :
                        singleResult.intensityScore >= 15 ? 'bg-amber-500' : 'bg-slate-400'
                      }`}
                ...
                <div className="space-y-2">
                  <h4 className="font-bold text-slate-700 text-xs uppercase tracking-wider">Kompetensi yang Terdeteksi (Skills Extracted)</h4>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-[180px] overflow-y-auto pr-1">
                      ...
                      return (
                        <div 
                          ...
                          className={`p-3 rounded-xl border flex flex-col justify-between ${
                            isDirect ? 'bg-emerald-50/40 border-emerald-100/80 text-emerald-900' :
                            isIndirect ? 'bg-blue-50/40 border-blue-100/80 text-blue-900' :
                            isTransition ? 'bg-amber-50/40 border-amber-100/80 text-amber-900' :
                            'bg-slate-50 border-slate-200/80 text-slate-800'
                          }`}
                        >
                          <div className="flex justify-between items-start">
                            <span className="font-bold text-xs truncate max-w-[150px]">{skill.name}</span>
                            <span className={`text-[9px] font-bold tracking-wider px-1.5 py-0.5 rounded uppercase ${
                              isDirect ? 'bg-emerald-100 text-emerald-800' :
                              isIndirect ? 'bg-blue-100 text-blue-800' :
                              isTransition ? 'bg-amber-100 text-amber-800' :
                          ...
                          <div className="mt-2 flex items-center justify-between text-[10px]">
                            <span className="text-slate-400">Confidence AI</span>
                            <span className="font-semibold">{(skill.confidence * 100).toFixed(0)}%</span>
                          </div>
                        </div>
                      );
                    })}
                    {singleResult.skills.length === 0 && (
                      <p className="text-xs text-slate-400 col-span-full italic text-center py-4">
                        Tidak ada kompetensi khusus yang terdeteksi dalam deskripsi pekerjaan ini.
                      </p>
                    )}
                  </div>
                </div>
                ...
                <div className="text-[10px] text-slate-400 leading-relaxed border-t border-slate-100 pt-3 flex gap-2 items-start">
                  ...
                  <div>
                    <span className="font-semibold text-slate-500 block">Cara Indeks Dihitung:</span>
                    Indeks dihitung dengan membandingkan bobot green skills (Direct = 1.5, Transition = 1.2, Indirect = 1.0) dengan total keseluruhan kompetensi yang tertulis pada lowongan kerja.
                  </div>
                </div>
              </div>
            ) : (
              <div className="bg-white rounded-2xl border border-slate-100 p-8 shadow-sm flex flex-col justify-center items-center text-center h-full min-h-[300px]">
                ...
                <h4 className="font-semibold text-slate-700 text-sm">Menunggu Ekstraksi Sandbox</h4>
                <p className="text-slate-400 text-xs max-w-sm mt-1">
                  Pilih lowongan preset bersih di sebelah kiri atau ketik deskripsi pekerjaan kustom Anda sendiri untuk memulai analisis NLP.
                </p>
              </div>
            )}
          </div>
