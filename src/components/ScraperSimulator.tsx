...
export default function ScraperSimulator({
  ...
  const handleStartScraping = () => {
    ...
    addLog(
      `System: Menargetkan web source: ${config.source === "All" ? "JobStreet, Glints, & LinkedIn" : config.source}`,
    );
    addLog(
      `System: Target kuota sampel: ${config.targetSampleSize} lowongan kerja.`,
    );
    addLog(`System: Filter wilayah East Java: ${config.locations.join(", ")}`);
    addLog(
      `System: Filter industri prioritas: ${config.industries.join(", ")}`,
    );
  };
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (currentStep >= maxSteps) {
        ...
        addLog(
          `System: Total Berhasil: ${stats.successCount + Math.min(pool.length, maxSteps)}, Gagal: 0.`,
        );
        addLog(
          "System: Data disimpan ke Raw Dataset Workspace (raw_jobs.csv).",
        );

        // Return scraped jobs to parent (take the sample from mock database)
        const finalSample = pool
          .slice(0, maxSteps)
          .map((j) => ({ ...j, is_cleaned: false }));
      ...
      if (currentStep === 0) {
        addLog(
          `Crawler: Mengirim GET request ke halaman pencarian ${jobItem.source} dengan query...`,
        );
      }
      ...
      if (success) {
        setStats((prev) => {
          ...
          updatedBySource[jobItem.source] =
            (updatedBySource[jobItem.source] || 0) + 1;
        ...
        addLog(
          `Crawler: [${jobItem.source}] Berhasil mengikis detail "${jobItem.job_title}" di ${jobItem.company} (${jobItem.location})`,
        );
      } else {
        setStats((prev) => ({
          ...prev,
        ...
        addLog(
          `Crawler: [${jobItem.source}] Gagal menyambung ke detail page. Mengulang koneksi...`,
        );
      }
    ...
    <div className="space-y-6" id="scraper-simulator-root">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
        ...
        <p className="text-slate-600 text-sm leading-relaxed">
          Gunakan modul ini untuk mensimulasikan web scraping secara aman.
          Crawler akan menjelajahi portal lowongan kerja utama Indonesia (
          <strong className="text-emerald-700">JobStreet & Glints</strong>)
          untuk mengumpulkan deskripsi pekerjaan, kualifikasi, tingkat
          pengalaman, dan rentang gaji.
        </p>
      </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm space-y-4">
          ...
          <div className="space-y-1">
            <label className="text-xs font-medium text-slate-500 block">
              Portal Sumber (Source Portal)
            </label>
            <select
              value={config.source}
              disabled={stats.isRunning}
              onChange={(e) =>
                setConfig({ ...config, source: e.target.value as any })
              }
            ...
            <div className="space-y-1">
              ...
              <input
                ...
                onChange={(e) =>
                  setConfig({
                    ...config,
                    delayMs: Math.max(100, parseInt(e.target.value) || 100),
                  })
                }
                className="w-full px-3 py-1.5 text-xs bg-slate-50 border border-slate-200 rounded-lg focus:outline-none"
              />
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-medium text-slate-500 block">
              Daftar Kata Kunci (Query Keywords)
            </label>
            <div className="flex flex-wrap gap-1 bg-slate-50 p-2 border border-slate-200 rounded-lg max-h-24 overflow-y-auto">
              {config.keywords.map((kw, i) => (
                <span
                  key={i}
                  className="px-1.5 py-0.5 bg-emerald-100 text-emerald-800 rounded text-[10px] font-medium border border-emerald-200/50"
                >
                  {kw}
                </span>
              ))}
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-medium text-slate-500 block">
              Prioritas Lokasi (East Java Cluster)
            </label>
            <div className="flex flex-wrap gap-1 bg-slate-50 p-2 border border-slate-200 rounded-lg">
              {config.locations.map((loc, i) => (
                <span
                  key={i}
                  className="px-1.5 py-0.5 bg-blue-100 text-blue-800 rounded text-[10px] font-medium border border-blue-200/50"
                >
                  {loc}
                </span>
              ))}
          ...
          {config.source === "LinkedIn" && (
            <div className="p-3 bg-amber-50 border border-amber-100 rounded-lg flex gap-2 items-start">
              ...
              <p className="text-[10px] text-amber-800 leading-normal">
                LinkedIn dilindungi sistem Cloudflare anti-bot yang ketat.
                Scraping LinkedIn hanya disarankan untuk benchmark validasi
                dengan volume kecil.
              </p>
            </div>
          )}
          ...
          <div className="grid grid-cols-3 gap-3 bg-white rounded-xl border border-slate-100 p-4 shadow-sm">
            ...
            <div className="text-center border-x border-slate-100">
              ...
              <span className="text-sm font-bold text-slate-700 mt-1 block">
                {stats.bySource.JobStreet} / {stats.bySource.Glints} /{" "}
                {stats.bySource.LinkedIn}
              </span>
            </div>
            <div className="text-center">
              <span className="text-[10px] font-medium text-slate-400 block uppercase">
